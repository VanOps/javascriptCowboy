...
import { llamarMCP, type Mensaje, type ModuloId } from "@/lib/mcp";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const tokenPreview =
      githubToken.substring(0, 10) + "..." + githubToken.slice(-4);
    console.log(
      `${TAG} Token detectado: ${tokenPreview} (${githubToken.length} chars)`,
    );

    const respuesta = await llamarMCP(mensajes, modulo, githubToken);

    console.log(
      `${TAG} ✅ Respuesta OK (${Date.now() - inicio}ms, ${respuesta.length} chars)`,
    );
